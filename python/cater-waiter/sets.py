...
def categorize_dish(dish_name, dish_ingredients):
    ...
    for category in ["VEGAN", "VEGETARIAN", "PALEO", "KETO", "OMNIVORE"]:
        food_group = getattr(sets_categories_data, category)
        if dish_ingredients.issubset(food_group):
            return f"{dish_name}: {category}"
...
def tag_special_ingredients(dish):
    ...
    """
    dish_name, dish_ingredients = dish  # unpack the tuple
    dangerous_ingredients = set(dish_ingredients).intersection(SPECIAL_INGREDIENTS)
    return (dish_name, dangerous_ingredients)
...
def compile_ingredients(dishes):
    ...
    all_ingredients = set()
    return all_ingredients.union(*dishes)

    # [{1, 2, 3}, {4, 5, 6}]

    # dishes as param:
    # all_ingredients.union([{1, 2, 3}, {4, 5, 6}])
...
def singleton_ingredients(dishes, intersection):
    ...
    for dish in dishes:
        undiscarded = set(dish).difference(intersection)
        singletons.update(undiscarded)
